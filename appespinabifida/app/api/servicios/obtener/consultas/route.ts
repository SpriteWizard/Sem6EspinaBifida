...
export async function GET(request: Request) {
  ...
  const res = await fetch(
    `https://g53bc679c5acb2c-espinabd.adb.mx-queretaro-1.oraclecloudapps.com/ords/admin/services/obtenerConsultas`,
    {
      method: "GET",
      headers: {
  ...
  const consultas = items.map((item: any) => ({
    ...item,
    tipo_servicio: "Consulta",
  }))

  const enriched = consultas.map((item: any) => {
    ...
    const rawFecha =
      item.FECHA ?? item.fecha ?? item.fecha_cita ?? item.fecha_creacion ?? item.fechaCreacion ?? item.fechacreacion ?? item.FECHA_CREACION
  ...
  const paged = filtrados.slice(cursor, cursor + limit).map((entry: { item: any }) => entry.item)
